...
while True:
    try:
        # 3. 변환하려는 금액 입력 받기 
        amount = float(input('변환할 금액을 입력해주세요: '))
        from_currency = input("어떤 화폐에서 변환하시곘습니까? (예: USD, KRW): ").upper()
        to_currency = input("어떤 화폐로 변환하시곘습니까? (예: USD, KRW): ").upper()

        # 4. 환율 계산하는 함수 호출
        a = currency_converter(amount, from_currency, to_currency)
        
        # 5. 결과 출력. from current는 to currency 입니다~
        print('변환 전 금액: {0} {1}'.format(amount, from_currency))
        print('변환 후 금액: {0} {1}'.format(a, to_currency))
